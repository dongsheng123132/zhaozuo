...
尚未完成：完整 UIA 树、多模态步骤理解、OCR、档案库、聊天软件会话流证据、代码签名。进程声明 per-monitor DPI 感知，坐标是物理像素；档案记录录制时的缩放，回放时不一致会被报出来。录制层仍是 20ms 轮询（无滚轮与拖拽），且尚未处理 DPI 缩放。当前步骤提炼使用本地规则，不应宣传为已经具备通用 AI 学习能力。
